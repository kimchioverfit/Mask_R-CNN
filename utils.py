...
def reduce_dict(input_dict, average=True):
    """Reduce values in the dictionary from all processes.
    ...
    Returns:
        dict: same keys as input_dict, reduced across processes
    """
    world_size = get_world_size()
    if world_size < 2:
        return input_dict
    ...
    with torch.inference_mode():
        ...
        # sort keys for consistent order across processes
...
class MetricLogger:
    ...
    def update(self, **kwargs):
        for k, v in kwargs.items():
            if isinstance(v, torch.Tensor):
                v = v.item()
            assert isinstance(v, (float, int)), f"{k} must be float/int, got {type(v)}"
    ...
    def __str__(self):
        parts = [f"{name}: {meter}" for name, meter in self.meters.items()]
