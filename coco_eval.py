...
class CocoEvaluator:
    def __init__(self, coco_gt, iou_types):
        ...
        self.coco_eval = {
            iou_type: COCOeval(self.coco_gt, iouType=iou_type)
    ...
    def synchronize_between_processes(self):
        for iou_type in self.iou_types:
            self.eval_imgs[iou_type] = np.concatenate(
                self.eval_imgs[iou_type], axis=2
            )
...
def merge(img_ids, eval_imgs):
    ...
    merged_eval_imgs = []
    for imgs in all_eval_imgs:
        merged_eval_imgs.append(imgs)

    merged_img_ids = np.array(merged_img_ids)
    merged_eval_imgs = np.concatenate(merged_eval_imgs, axis=2)

    merged_img_ids, idx = np.unique(
        merged_img_ids, return_index=True
    )
...
def evaluate(coco_eval):
    ...
    return coco_eval.params.imgIds, eval_imgs
